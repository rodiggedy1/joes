...
import "./accountBookingOverlay.css";
...
export default function Account() {
  ...
  const visibleServices = showAllServices ? additionalServices : additionalServices.slice(0, 4);
  const openAccountBooking = (request = "") => { setBookingRequest(`I need help with ${request}.`); setBookingChatOpen(true); };
  ...
  return <main className="portal-shell account-shell">
    ...
    <section className="account-hero"><div><div className="portal-eyebrow">MY HOME · GOOD JOE</div><h1>Welcome back{user?.name ? `, ${user.name.split(" ")[0]}` : ""}.</h1><p>Your home requests, timing, and updates are all here.</p></div><button className="portal-primary" onClick={() => openAccountBooking()}><Plus /> Start a new request</button></section>
    ...
    <section className="account-discovery" aria-labelledby="account-discovery-heading"><div className="section-heading"><div><div className="portal-eyebrow">MORE JOE CAN HANDLE</div><h2 id="account-discovery-heading">What else can we take off your list?</h2></div><button className="account-discovery-toggle" type="button" onClick={() => setShowAllServices(open => !open)} aria-expanded={showAllServices}>{showAllServices ? "Show fewer services" : "View all services"} <ArrowRight /></button></div><div className="account-discovery-grid">{visibleServices.map(service => <button className="account-discovery-card" type="button" onClick={() => openAccountBooking(service.request)} key={service.label}><div className={`account-discovery-icon ${service.tone}`}><service.icon /></div><div className="account-discovery-copy"><strong>{service.label}</strong><p>{service.description}</p><span>Get started <ArrowRight /></span></div></button>)}</div></section>
    <section className="account-section"><div className="section-heading"><div><div className="portal-eyebrow">YOUR BOOKINGS</div><h2>Everything you’ve asked Joe to handle.</h2></div>{bookings.length > 0 && <span className="record-count">{bookings.length} {bookings.length === 1 ? "booking" : "bookings"}</span>}</div>{bookingsQuery.isLoading ? <div className="portal-empty"><Sparkles /><p>Loading your booking history…</p></div> : bookings.length === 0 ? <div className="portal-empty account-empty"><div className="portal-icon"><ClipboardList /></div><h3>Your booking history starts here.</h3><p>When you book with Good Joe, the service, timing, and updates will appear in this account.</p><a className="portal-primary" href="/">Tell Joe what you need <ChevronRight /></a></div> : <div className="account-bookings">{bookings.map(({ booking, events }) => {
      ...
      return <article className="account-booking" key={booking.id}>
        <div className="booking-card-top"><div><div className="booking-service">{booking.service}</div><h3>{booking.title}</h3><span className={`status-pill status-${booking.status}`}>{statusCopy[booking.status]}</span>{booking.estimateRequiresReview && <span className="estimate-review-note">Estimate needs confirmation</span>}</div><div className="booking-price"><span>{booking.scopeSelections ? "Estimated total" : "Starting at"}</span><strong>{currency(booking.quotedCents)}</strong></div></div>
    ...
    <section className="account-help"><div className="portal-icon"><MessageCircle /></div><div><h3>Need to change something?</h3><p>Ask Joe in plain language. Your account keeps the whole request together.</p></div><button type="button" onClick={() => openAccountBooking()}>Ask Joe <ChevronRight /></button></section>
    <BookingAssistant open={bookingChatOpen} requestText={bookingRequest} onClose={() => setBookingChatOpen(false)} />
  </main>;
}
