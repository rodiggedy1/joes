...
const serviceCards = [
  ["🧹", "House cleaning", "Standard, deep, move-in, move-out and recurring cleaning."],
  ["🔨", "Handyman", "Small repairs, punch lists, hanging, patching and installations."],
  ["📺", "TV mounting", "TVs, soundbars, shelves and clean cable-management setups."],
  ["🪑", "Furniture assembly", "Beds, desks, dressers, shelving and outdoor furniture."],
  ["🌿", "Lawn & yard care", "Mowing, trimming, cleanup and routine outdoor maintenance."],
  ["🚚", "Moving help", "Loading, unloading, heavy lifting and in-home furniture moves."],
  ["🗑️", "Junk removal", "Furniture, appliances, garage cleanouts and unwanted items."],
  ["💦", "Pressure washing", "Driveways, patios, walkways, siding and outdoor surfaces."],
];
      ...
      <div className="site">
        ...
        <section className="hero">
          ...
          <div className="embed">
            ...
            <button className="btn wide" onClick={askEmbedded}>Ask Joe&apos;s AI →</button>
        ...
        <section className="services-intro" id="services"><div className="eyebrow">Popular services</div><h2>One place for the jobs around your home.</h2><p>From recurring upkeep to the random thing that needs fixing today. Tell Joe’s AI what you need and it’ll guide you from request to booked.</p></section>
        <section className="services">{serviceCards.map(([icon, title, copy]) => <div className="service" key={title}><span className="arrow">↗</span><div className="icon">{icon}</div><h3>{title}</h3><p>{copy}</p></div>)}</section>
        <section className="anything"><div><div className="eyebrow">Don’t see it?</div><h3>Just ask. That’s the point.</h3><p>Describe the job in your own words. If it’s something we can get handled, Joe’s AI will take it from there.</p></div><button className="btn" onClick={() => openChat()}>Tell us what you need →</button></section>
        ...
        <section className="coverage"><div className="coverage-head"><div><div className="eyebrow">More ways we can help</div><h2>Your home has a long to-do list.</h2></div><p>Joe’s AI is designed around the way people actually ask for help — by describing the problem, not hunting through a directory.</p></div><div className="category-pills">{categories.map((category) => <span key={category}>{category}</span>)}<span className="more">+ whatever’s next</span></div></section>
        ...
        <footer><div className="brand"><div className="logo">J</div><strong>Joe&apos;s Home Services</strong></div><span>Cleaning · Handyman · Home Care</span><span>Secure booking · Insured professionals</span></footer>
      </div>
